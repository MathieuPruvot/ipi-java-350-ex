...
public class EmployeServiceTest {
    ...
    public void calculPerformanceCommercial(String matricule, Long caTraite, Long objectifCa, Integer performance, Double avgPerformance) throws EmployeException {
        //Given
	    Employe employeTest = new Employe("Doe", "Jhon", matricule, LocalDate.now(), Entreprise.SALAIRE_BASE, 2, 1d);
	    when(employeRepository.findByMatricule(matricule)).thenReturn(employeTest);
	    when(employeRepository.save(employeTest)).thenReturn(employeTest);
	    when(employeRepository.avgPerformanceWhereMatriculeStartsWith(any())).thenReturn(avgPerformance);
	    
	    //When
	    employeService.calculPerformanceCommercial(matricule,caTraite,objectifCa);
	    
	    //Then
        Assert.assertEquals("performance calculé erroné", performance, employeTest.getPerformance());
    }
